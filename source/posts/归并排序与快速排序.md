...
当时我还在背模板阶段，归并和快排算是第一批真正需要"理解"的排序。光看"递归"俩字看得头大，后来亲手把数组拆开再拼回去，才明白分治就是拆到最小再一步步合并。这篇把两个排序的实现和关键点记下来。
...
求逆序对的过程，说白了正好和归并排序对应：利用分治思想，逆序对数等于左半边逆序对数+右半边逆序对数+跨边逆序对数，正好对应"左半边排序+右半边排序+合并数组"这三步。当时我就是把这点想通了，代码才不再靠死背。
