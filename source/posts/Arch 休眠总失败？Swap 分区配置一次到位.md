...
给 Arch 折腾休眠的那天，我把 Swap 相关的东西全试了一遍都没成，最后才搞明白问题出在系统自带的 zram 上——它活在内存里，断电就没了，根本没法存休眠镜像。折腾完把整个流程记下来，给同样卡在休眠上的朋友少走点弯路。

# Arch Linux 配置 Swap 分区并启用休眠

---

先说结论：我那次为了休眠折腾了一整天，最后发现问题全出在 Swap 上——之前图省事用的 zram，根本扛不住休眠。下面把我从头到尾的配置过程记下来，照着走一遍基本就能通。
