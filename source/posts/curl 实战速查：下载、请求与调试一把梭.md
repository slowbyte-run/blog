...
最早用 curl 是去摸服务器上某个接口通没通，一条 `curl url` 就搞定，后来下载文件、带请求头、看响应调试全靠它。它的参数看着多，真正常用的也就那么几个。这篇按我的使用习惯把 curl 的常用场景整理成一份速查。
...
日常摸服务器，我用的最多的就是 curl：下载软件、测接口、看响应头，全靠它。刚上手那会儿光记参数就头大，`-o` 和 `-O`、`-s` 和 `-S`，全是字母大小写的区别，稍不留神就翻车。这篇是我把常用场景整理出的速查，从下载到调试一把梭。
