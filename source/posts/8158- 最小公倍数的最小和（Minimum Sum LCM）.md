...
根据数论知识，我们知道，一个大于等于2的整数均可以被拆分成有限个质数的乘积。也即当\(n >= 2\)时，有\(n = p_1^{a_1} \times p_2^{a_2} \times \cdots \times p_k^{a_k}\)  
那么如果我们将每一部分拿开来，它们的最小公倍数就是\(n\)，那如何使得它们的和最小呢？  
我们可以特殊化，从简单的二维来讨论。易知\(m, n \geq 2\)时，有\(m \times n \geq m + n\)。所以类比到多项的情况，我们可以猜测每个质因子 \(p_i^{a_i}\)单独地存在于一个数字中，答案是最优的。

## 注意

当\(n == 1\)时，由于至少需要分为两项，所以\(n = 1 \times n, ans = 1 + n\)  
当n只有一个质因子时， \(n = p_i^{a_i} \times 1, ans = 1 + p_i^{a_i}\)
