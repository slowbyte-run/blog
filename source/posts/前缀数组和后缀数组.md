...
前缀数组和后缀数组是我刷题时最早学会的一批“省钱”技巧，很多查询看着要循环，存一个前缀和就能 O(1) 出答案。这篇结合 5 个例题，记一下我对这两种技术的理解。

## 一、前缀数组（Prefix Sum Array）

### 基本概念

前缀数组说白了就是把从头到每个位置的和（或其他累积结果）提前算好存起来，之后想查任意区间，都是 O(1) 的事。
...
前缀数组和后缀数组是我刷区间题时最常用的两个工具，它们能做到：
...
这 5 道例题做下来，我对前缀/后缀数组才算真正用熟。比赛里别死记模板，题目一变就得灵活变通，很多时候还要把这两个技术组合起来用。
