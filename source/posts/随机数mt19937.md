...
当时写题想生成大范围的随机数，`rand()` 那点随机性根本不够看。后来换成了 mt19937 这套模板，顺手把每一行到底在干嘛也搞清楚了。这篇就按我的理解拆一遍。
...
**总结**：合起来看，就是拿当前“稳步时钟”的时间当作一个大整数，丢给随机数引擎当种子（seed）。
...
- 构造时传一个 `unsigned long long` 类型的种子，我习惯用时间戳，保证每次运行生成的序列都不一样。
...
- **注意**：别在循环里反复 re-seed（比如每轮都用时间戳重新初始化 rng），多次快速调用时时间戳可能相同，反而生成一堆重复的“随机”数。

---

所以最后真正要记住的，其实就是下面这两行：
