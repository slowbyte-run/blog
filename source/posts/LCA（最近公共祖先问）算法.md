...
这篇是自己整理的 LCA 笔记。最早在树上求两个点的最近公共祖先，我只会一个节点一个节点往上爬，查询一多就超时，后来学会倍增（二分提升）才顺了。下面记一下思路和个人的类模板。
...
LCA（最近公共祖先）看着只是个“公共节点”查询，其实树论、路径查询和树形 DP 里到处都是它的影子。我按大类把常见应用场景记一下：
...
- **两点距离**
  ...
  $dist(u,v)=depth[u]+depth[v]−2 depth[LCA(u,v)]. text{dist}(u,v) = text{depth}[u] + text{depth}[v] - 2\,text{depth}[mathrm{LCA}(u,v)]$
...
我的经验是，题目里出现“树上两点路径”“祖先关系判断”“路径汇总”或“子树合并”这类字眼时，LCA 多半就排得上用场。把它吃透，后面啃树形结构的高级题会顺手很多。

## 拓展

想再进一步的话可以研究「$Euler Tour + RMQ」$方法，能实现 O(1) 查询，不过预处理更复杂，我当时也只是先把这条思路记下。
