...
我之前切分支全靠 stash：改一半、存起来、切走、再切回来 pop，结果十次有八次 pop 出一地冲突，改的东西还得手动合并。后来知道有 worktree 这玩意儿——同一个仓库同时开好几个工作目录、各干各的分支互不干扰，才觉得自己以前是真傻。这篇就把它的基本用法和我的工作流记下来。
...
先交代下背景。我第一次被 worktree 救回来，是那次功能写一半，突然被喊去改线上 bug。切来切去、stash 来 stash 去，最后 pop 出一地冲突，差点想砸键盘。后来才知道 Git 本来就有 worktree 这个功能：同一个仓库开好几个工作目录，各干各的，互不打扰。这篇把我踩坑之后整理的用法从头到尾过一遍。

## 1. 什么是 Git Worktree

说白了，Git Worktree 就是允许你从**同一个 Git 仓库**中，在**不同的目录**里同时检出**多个分支**。每个目录称为一个 **worktree（工作树）**，它们共享同一个 `.git` 数据库。
...
先说说我以前最头疼的一个场景：活儿没干完，又得切分支，来回折腾。
...
比如我正埋头在 `feature/x` 上写功能，线上突然报了个 bug 要马上修：
...
把我一天的典型流程串起来，大概就是下面这样（也是我现在每天都在用的）：
